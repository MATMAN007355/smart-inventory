...
const Dashboard = () => {
  ...
  const [reorderQty, setReorderQty] = useState(10);
  const [reorderSuccess, setReorderSuccess] = useState(false);
  const [insightTab, setInsightTab] = useState("restock"); 

  // Backend AI Predictions state trackers
  ...
  // Fetch Live AI predictions from your backend endpoint via your apiClient instance
  useEffect(() => {
    const fetchPredictions = async () => {
      try {
        ...
        console.error("Error retrieving dashboard ai-predictions route:", err);
        setAiPredictions([]); 
      } finally {
  ...
  // Process data for "What is selling the most"
  const topSellingProducts = (() => {
    ...
    salesData.forEach((sale) => {
      ...
      const rev = Number(sale.totalValue) || 0;
      if (!productMap[name]) {
        productMap[name] = { product_name: name, sku: sale.sku || "N/A", totalSold: 0, revenue: 0 };
      }
      productMap[name].totalSold += qty;
      productMap[name].revenue += rev;
    });
    return Object.values(productMap).sort((a, b) => b.totalSold - a.totalSold);
  })();

  const restockList = [...(outOfStockItems || []), ...(lowStockItems || [])];
  ...
  }, [showAiDrawer, aiMessages.length, lowStockItems, totalInventoryValue]);
  ...
  const handleSendMessage = (e) => {
    ...
    setTimeout(() => {
      ...
      if (
        ...
        setShowAddProduct(true);
        setShowAiDrawer(false);
        reply = "I've launched the 'Add Product' workspace for you.";
        toast.info("Opening Add Product workspace...");
      } else if (
        normalInput.includes("fix low stock") ||
        normalInput.includes("reorder most critical")
        ...
        if (lowStockItems && lowStockItems.length > 0) {
          ...
          reply = "All inventory items are comfortably above safety thresholds.";
        }
      }
  ...
  const inputClass = "w-full px-4 py-2 border border-slate-600 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 bg-slate-700 text-slate-100 placeholder-slate-400 disabled:opacity-50";
  const totalMovements = stockMovements ? stockMovements.length : 0;
  const donutTotal = donutData ? donutData.reduce((s, d) => s + d.value, 0) : 0;
    ...
    <main className="flex-1 bg-slate-800 p-8 relative min-h-screen transition-all duration-300">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-slate-900 p-5 rounded-xl border border-slate-700">
          <p className="text-xs text-slate-400 mb-1">Total Inventory Value</p>
          <p className="text-2xl font-bold text-slate-100">
            {totalInventoryValue > 0 ? fmtCurrency(totalInventoryValue) : "No Data"}
          </p>
          <p className="text-xs text-emerald-400 mt-1">Live from products</p>
        </div>
        <div className="bg-slate-900 p-5 rounded-xl border border-slate-700">
          <p className="text-xs text-slate-400 mb-1">Total Products</p>
          <p className="text-2xl font-bold text-slate-100">
            {totalProducts > 0 ? totalProducts.toLocaleString() : "No Data"}
          </p>
          <p className="text-xs text-emerald-400 mt-1">Live from products</p>
        </div>
        <div className="bg-slate-900 p-5 rounded-xl border border-slate-700">
        ...
        <div className="bg-slate-900 p-5 rounded-xl border border-slate-700">
          <p className="text-xs text-slate-400 mb-1">Stock Movements</p>
          <p className="text-2xl font-bold text-slate-100">
            {totalMovements > 0 ? totalMovements : "No Data"}
          </p>
          <p className="text-xs text-emerald-400 mt-1">This session</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-1 bg-slate-900 p-6 rounded-xl border border-slate-700 h-72">
          ...
          {lineData && lineData.length > 0 ? (
            ...
            <div className="h-[80%] flex items-center justify-center text-xs text-slate-500 italic">No chart data available</div>
          )}
        </div>

        {/* Donut Chart */}
        <div className="bg-slate-900 p-6 rounded-xl border border-slate-700 h-72 flex flex-col justify-between">
          <div>
            <h2 className="text-sm font-semibold text-slate-200 mb-2">Stock Status Overview</h2>
            {donutData && donutData.length > 0 && donutTotal > 0 ? (
              <div className="flex items-center gap-2">
                <PieChart width={130} height={130}>
                ...
                <ul className="space-y-2 text-xs text-slate-400 flex-1 overflow-y-auto max-h-[130px]">
                  {donutData.map((d) => (
                    <li key={d.name} className="flex justify-between gap-2">
                      <span style={{ color: d.color }} className="truncate">■ {d.name}</span>
                      <span className="shrink-0">{d.value} ({((d.value / donutTotal) * 100).toFixed(1)}%)</span>
                    </li>
                  ))}
                </ul>
              </div>
            ) : (
              <div className="py-12 text-center text-xs text-slate-500 italic">No dynamic stock chart statuses</div>
            )}
          </div>
          <div className="flex justify-between text-sm font-semibold text-slate-200 border-t border-slate-700 pt-3">
            <span>Total Status Items</span>
            <span>{donutTotal > 0 ? donutTotal : "No Data"}</span>
          </div>
        </div>

        {/* Recent Alerts Panel AND Backend AI Predictions Route Display Container */}
        <div className="bg-slate-900 p-6 rounded-xl border border-slate-700 h-72 overflow-y-auto space-y-4">
          <div>
            <h2 className="text-sm font-semibold text-indigo-400">⚡ Live Backend AI Insights</h2>
            {predictionsLoading ? (
              <p className="text-xs text-slate-500 italic mt-2 animate-pulse">Running smart system diagnostics...</p>
            ) : aiPredictions.length > 0 ? (
              <ul className="mt-2 space-y-2 text-xs text-slate-300">
                {aiPredictions.map((pred, i) => (
                  <li key={i} className="p-2 bg-slate-800 rounded-lg border border-slate-700 flex flex-col gap-1">
                    <span className="font-semibold text-slate-200">{pred.insight || "System Alert Recommendation"}</span>
                    <span className="text-[10px] text-slate-400">{pred.details || "Action needed immediately to keep operations running smoothly."}</span>
                  </li>
                ))}
              </ul>
            ) : (
              <p className="text-xs text-slate-500 italic mt-2">No Data — No automated predictive warnings generated yet</p>
            )}
          </div>

          <div className="border-t border-slate-800 pt-2">
            <div className="flex items-center justify-between mb-2">
              <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Critical Anomalies</h2>
            </div>
            {lowStockItems?.length === 0 && outOfStockItems?.length === 0 ? (
              <div className="text-xs text-slate-500 italic">No stock alerts found</div>
            ) : (
              <ul className="space-y-2 text-xs text-slate-400">
                {lowStockItems?.length > 0 && (
                  <li className="flex justify-between items-center bg-slate-800/40 p-1.5 rounded">
                ...
                {outOfStockItems?.length > 0 && (
                  <li className="flex justify-between items-center bg-slate-800/40 p-1.5 rounded">
                    <span>{outOfStockItems.length} items completely depleted</span>
      ...
      {/* =========================================================================
          3. INSIGHTS GRID: WHAT TO RESTOCK vs WHAT IS SELLING MOST
          ========================================================================= */}
      <div className="bg-slate-900 p-6 rounded-xl border border-slate-700 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-4 mb-4 gap-3">
          <div className="flex items-center gap-4">
            <button 
              onClick={() => setInsightTab("restock")} 
              className={`text-sm font-semibold pb-1 transition-all ${insightTab === "restock" ? "text-indigo-400 border-b-2 border-indigo-400" : "text-slate-400 hover:text-slate-200"}`}
            >
              ⚠️ What to Restock
            </button>
            <button 
              onClick={() => setInsightTab("selling")} 
              className={`text-sm font-semibold pb-1 transition-all ${insightTab === "selling" ? "text-indigo-400 border-b-2 border-indigo-400" : "text-slate-400 hover:text-slate-200"}`}
            >
              🔥 What is Selling the Most
            </button>
          </div>
          <span className="text-xs text-slate-500">Real-time analytical cross-match</span>
        </div>
        ...
        <div className="overflow-x-auto">
          {insightTab === "restock" ? (
            <table className="w-full text-sm text-slate-400">
              <thead className="text-xs text-slate-500 border-b border-slate-800">
                <tr>
                  ...
                  <th className="text-left pb-2">SKU Reference</th>
                  <th className="text-left pb-2">Current Count</th>
                  <th className="text-left pb-2">Status Flag</th>
                  <th className="text-right pb-2">Action Required</th>
                </tr>
              </thead>
                ...
                {restockList.length > 0 ? (
                  restockList.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-800/40">
                      <td className="py-3 text-slate-200 font-medium">{item.product_name}</td>
                      <td className="py-3 text-xs font-mono text-slate-500">{item.unique_code || "N/A"}</td>
                      <td className="py-3 text-red-400 font-bold">{item.quantity}</td>
                      <td className="py-3">
                        <span className={`text-[11px] px-2 py-0.5 rounded-full border ${item.quantity === 0 ? "bg-red-950/40 text-red-400 border-red-900/50" : "bg-amber-950/40 text-amber-400 border-amber-900/50"}`}>
                          {item.quantity === 0 ? "Out of Stock" : "Low Stock Level"}
                        </span>
                      </td>
                      <td className="py-3 text-right">
                        <button onClick={() => setReorderItem(item)} className="text-xs bg-indigo-600/10 border border-indigo-500/20 px-2.5 py-1 rounded-lg text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all">
                          Restock Item
                        </button>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan={5} className="py-10 text-center text-slate-500 text-xs italic">
                      No Data — No restock recommendations active (All items healthy)
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <table className="w-full text-sm text-slate-400">
              <thead className="text-xs text-slate-500 border-b border-slate-800">
                <tr>
                  <th className="text-left pb-2">Top Product</th>
                  <th className="text-left pb-2">SKU</th>
                  <th className="text-center pb-2">Total Units Sold</th>
                  <th className="text-right pb-2">Generated Revenue</th>
                  <th className="text-right pb-2">Velocity Rank</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60">
                {topSellingProducts.length > 0 ? (
                  topSellingProducts.slice(0, 5).map((item, index) => (
                    <tr key={index} className="hover:bg-slate-800/40">
                      ...
                      <td className="py-3 text-center text-emerald-400 font-semibold">{item.totalSold} units</td>
                      <td className="py-3 text-right text-slate-200 font-medium">{fmtCurrency(item.revenue)}</td>
                      <td className="py-3 text-right">
                        <span className="bg-emerald-950/60 text-emerald-400 text-xs px-2 py-0.5 rounded border border-emerald-900/30 font-bold">
                          # {index + 1} Top Seller
                        </span>
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan={5} className="py-10 text-center text-slate-500 text-xs italic">
                      No Data — No sales transactions recorded to generate velocity metrics
                    </td>
                  </tr>
                )}
              </tbody>
                ...
                {filteredLowStock && filteredLowStock.length > 0 ? (
                  filteredLowStock.slice(0, 5).map((item) => (
                    <tr key={item.unique_code || item.product_name} className="hover:bg-slate-800">
                      ...
                      <td className="py-3">
                        <span className="bg-amber-950/80 text-amber-400 text-xs px-2 py-0.5 rounded-full border border-amber-800/40">
                          {item.quantity === 0 ? "Out of Stock" : "Low Stock"}
                        </span>
                      </td>
                      <td className="py-3">
                        <button onClick={() => setReorderItem(item)} className="text-xs border border-slate-600 px-2 py-1 rounded-lg text-slate-300">Reorder</button>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-500 text-xs italic">No Data — No low stock items detected</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-slate-400">
              ...
              <tbody className="divide-y divide-slate-800">
                {salesLoading ? (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-500 text-xs">Loading ledger feeds...</td>
                  </tr>
                ) : salesData && salesData.length > 0 ? (
                  salesData.slice(0, 5).map((t, i) => (
                    <tr key={t.reference || i} className="hover:bg-slate-800">
                      <td className="py-3">
                        <p className="text-slate-200 truncate max-w-[120px] font-medium">{t.product}</p>
                        <p className="text-xs text-slate-500 font-mono">{t.sku || "N/A"}</p>
                      </td>
                      <td className="py-3 text-xs text-slate-400 capitalize">{t.location || t.category || "General"}</td>
                      ...
                      <td className="py-3 text-xs text-slate-500 whitespace-nowrap">{t.time || "Just now"}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-500 text-xs italic">No Data — No transactional logs recorded yet</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {/* =========================================================================
          5. QUICK ACTIONS FOOTER PANEL
          ========================================================================= */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div onClick={() => setShowAddProduct(true)} className="bg-slate-900 p-5 rounded-xl border border-slate-700 flex items-center gap-4 cursor-pointer hover:bg-slate-800 transition-all">
          <div className="bg-indigo-950 p-3 rounded-xl text-xl">➕</div>
          <div>
            <p className="text-sm font-semibold text-slate-200">Add Product</p>
        ...
        <div onClick={() => setShowCreatePO(true)} className="bg-slate-900 p-5 rounded-xl border border-slate-700 flex items-center gap-4 cursor-pointer hover:bg-slate-800 transition-all">
          <div className="bg-indigo-950 p-3 rounded-xl text-xl">📋</div>
          <div>
            <p className="text-sm font-semibold text-slate-200">Create PO</p>
            <p className="text-xs text-slate-500">Create new purchase order</p>
        ...
        <div onClick={() => navigate("/transactions")} className="bg-slate-900 p-5 rounded-xl border border-slate-700 flex items-center gap-4 cursor-pointer hover:bg-slate-800 transition-all">
          <div className="bg-indigo-950 p-3 rounded-xl text-xl">📦</div>
          <div>
            <p className="text-sm font-semibold text-slate-200">Stock In</p>
        ...
        <div onClick={() => navigate("/reports")} className="bg-slate-900 p-5 rounded-xl border border-slate-700 flex items-center gap-4 cursor-pointer hover:bg-slate-800 transition-all">
          <div className="bg-indigo-950 p-3 rounded-xl text-xl">📊</div>
          <div>
            <p className="text-sm font-semibold text-slate-200">Reports</p>
            <p className="text-xs text-slate-500">View inventory reports</p>
      ...
      {/* =========================================================================
          6. FLOATING CO-PILOT ASSISTANT DRAWER
          ========================================================================= */}
      <div className={`fixed inset-y-0 right-0 w-80 md:w-96 bg-slate-900 border-l border-slate-700 z-50 shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out ${showAiDrawer ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="p-2 border-t border-slate-800/60 bg-slate-950/20 flex flex-wrap gap-1.5 px-3">
          <button type="button" onClick={() => setAiInput("Add product")} className="text-[11px] bg-slate-800 border border-slate-700 text-indigo-400 px-2 py-1 rounded-md">➕ Launch Add Wizard</button>
          <button type="button" onClick={() => setAiInput("Fix low stock")} className="text-[11px] bg-slate-800 border border-slate-700 text-amber-400 px-2 py-1 rounded-md">⚠️ Auto-Reorder Critical</button>
        </div>
        ...
        <form onSubmit={handleSendMessage} className="p-3 bg-slate-950/60 border-t border-slate-700 flex gap-2">
          <input
            ...
            onChange={(e) => setAiInput(e.target.value)}
            placeholder="Ask anything about inventory..."
            className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-100 outline-none focus:ring-1 focus:ring-indigo-500"
          />
          <button type="submit" className="bg-indigo-600 text-white px-3 py-2 rounded-xl text-xs font-medium hover:bg-indigo-500">Send</button>
        </form>
      </div>

      {/* Floating Toggle Button */}
      <button onClick={() => setShowAiDrawer(true)} className="fixed bottom-6 right-6 z-40 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-medium px-5 py-3 rounded-full shadow-2xl hover:scale-105 flex items-center gap-2 border border-indigo-400/30">
        <span className="text-lg">✨</span> Ask AI Assistant
      </button>
            ...
            {reorderSuccess ? (
              ...
              <>
                ...
                <div className="mb-6">
                  ...
                  <input type="number" min={1} value={reorderQty} onChange={(e) => setReorderQty(e.target.value)} className={inputClass} />
                </div>
                <div className="flex items-center gap-3">
          ...
          <div className="relative w-full max-w-md p-6 overflow-hidden rounded-2xl bg-[#0F172B] border border-slate-800 shadow-2xl z-10">
            <form onSubmit={handleAddProductConfirm}>
              <div className="flex items-center justify-between mb-6">
                ...
                <button type="button" disabled={isAddingProduct} onClick={() => setShowAddProduct(false)} className="p-1 text-xl text-slate-400 rounded-lg">✕</button>
              </div>
              <div className="space-y-4 mb-6">
                <div>
                  <label className="block mb-1 text-xs font-medium text-slate-400">Product Name</label>
                  <input type="text" required disabled={isAddingProduct} placeholder="e.g. Paracetamol" value={newProduct.product_name} onChange={(e) => setNewProduct({ ...newProduct, product_name: e.target.value })} className={inputClass} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <input type="number" required disabled={isAddingProduct} min={0} placeholder="5000" value={newProduct.price} onChange={(e) => setNewProduct({ ...newProduct, price: e.target.value })} className={inputClass} />
                  </div>
                </div>
                <div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <button type="button" disabled={isAddingProduct} onClick={() => setShowAddProduct(false)} className="flex-1 px-4 py-2.5 text-sm border border-slate-700 rounded-xl text-slate-300 hover:bg-slate-800">Cancel</button>
                <button type="submit" disabled={isAddingProduct} className="flex-1 px-4 py-2.5 text-sm text-white bg-indigo-600 rounded-xl hover:bg-indigo-500 shadow-lg">
                  {isAddingProduct ? "Saving..." : "Add Product"}
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl shadow-xl p-8 w-full max-w-md">
            {createPOSuccess ? (
              <div className="flex flex-col items-center justify-center gap-3 py-6">
                ...
                <p className="text-slate-400 text-sm">Your purchase order has been submitted successfully.</p>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between mb-6">
