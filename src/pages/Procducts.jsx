...
import apiClient from '../api/client'; // Path aligned to your import location
...
const Products = () => {
  ...
  // Fetch products from backend Mongoose architecture
  ...
  // 1. VIEW FUNCTION: Fetch single object data context from backend
  ...
  // 2. EDIT BUTTON INITIALIZER: Set active object and fill form state
  ...
  // 2b. EDIT SUBMITTER: Dispatch transaction data to MongoDB patch endpoint
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await apiClient.put(`/products/${editingProduct.unique_code}`, editForm);
      if (response.data && response.data.success) {
        // Update local array data matching mutated row changes
        setProducts(products.map(p => p.unique_code === editingProduct.unique_code ? response.data.data : p));
  ...
  // 3. DELETE FUNCTION: Wipe item document from DB state collection
    ...
    if (window.confirm('Are you sure you want to permanently delete this product document?')) {
      try {
        ...
        if (response.status === 200 || response.data?.success) {
          const freshDataset = products.filter(product => product.unique_code !== code);
          setProducts(freshDataset);
          
          // Re-evaluate page offset index parameters
          const updatedTotalPages = Math.ceil(freshDataset.length / itemsPerPage);
  ...
  // Pagination computational mapping logic
  ...
  return (
    <div style={styles.pageBackground}>
      <div style={styles.container}>
              ...
              {currentItems.length > 0 ? (
                currentItems.map((product, index) => (
                  <tr key={product._id} style={styles.tr}>
                    {/* Dynamic Pagination S/N Counter calculation */}
                    ...
                    <td style={styles.td}>
                      <span style={product.quantity > 0 ? styles.badgeIn : styles.badgeOut}>
        ...
        {/* ==========================================
            INLINE VIEW MODAL ELEMENT OVERLAY
           ========================================== */}
        {viewingProduct && (
        ...
        {/* ==========================================
            INLINE EDIT FORM MODAL OVERLAY
           ========================================== */}
        {editingProduct && (
          <div style={styles.modalOverlay}>
            <div style={styles.modalContent}>
...
// Styling profiles supporting background-color: bg-slate-800 (#1e293b)
const styles = {
  pageBackground: {
  ...
  tdNum: {
    padding: '16px 16px 16px 24px',
    fontSize: '14px',
    color: '#64748b', // Dimmer color for index numbers
    fontWeight: '500',
  },
  codeTag: {
  ...
  // Modal layout configurations styles mappings
  modalOverlay: {
    position: 'fixed',
    top: 0,
    left: 0,
